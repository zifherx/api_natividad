...
export class CreateTables1718424462164 implements MigrationInterface {
    name = 'CreateTables1718424462164'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`Tbl_Users\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`firstName\` varchar(30) NOT NULL, \`lastName\` varchar(30) NOT NULL, \`cellphone\` varchar(9) NULL, \`document\` varchar(15) NOT NULL, \`email\` varchar(50) NOT NULL, \`username\` varchar(50) NOT NULL, \`password\` varchar(65) NOT NULL, \`avatar\` varchar(255) NULL, \`isActive\` tinyint NOT NULL DEFAULT '1', \`isDeleted\` tinyint NOT NULL DEFAULT '0', \`typeDocumentId\` varchar(36) NULL, \`roleId\` varchar(36) NULL, UNIQUE INDEX \`IDX_27367103756b0d8e84407f94f4\` (\`email\`), UNIQUE INDEX \`IDX_a9768445097f20f1205db5ae5d\` (\`username\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`Tbl_Users\` ADD CONSTRAINT \`FK_7be2ebad37524cd08167a63a5e5\` FOREIGN KEY (\`roleId\`) REFERENCES \`Tbl_Roles\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
    }
